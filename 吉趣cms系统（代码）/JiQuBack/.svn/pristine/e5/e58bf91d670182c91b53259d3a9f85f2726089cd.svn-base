			...
			$.ajax({
				...
				"data":{
			  		"filter": {
				    	...
				    	"fields":["id","actName","actOrgan","actLoca","actBrief","actState","actTime","SignPeople"],
					}
				},
				"cache": false,
				"headers": apiHeader,
			...
			if (ret.actState==0) {
				strBox=[
							'<tr>',
				                ...
				                '<th>'+ret.actTime+'</th>',
				                '<th>'+ret.actLoca+'</th>',
				                '<th>'+ret.SignPeople.length+'</th>',
				                '<th><button type="button" class="btn btn-info btn-xs">未开始</button></th>',
				                '<th>',			                	
				                	'<a href="#signCheck_Checkpeople?id='+ret.id+'"><button type="button" class="btn btn-primary btn-sm">查看签到结果</button></a>',
				                '</th>',
			            	'</tr>'
			            	];
				...
				strBox=[
						'<tr>',
				            ...
				            '<th>'+ret.actLoca+'</th>',
				            '<th>'+ret.SignPeople.length+'</th>',
			                '<th><button type="button" class="btn btn-warning btn-xs">进行中</button></th>',
			                '<th>',			                	
			                	'<a href="#signCheck_Checkpeople?id='+ret.id+'"><button type="button" class="btn btn-primary btn-sm">查看签到结果</button></a>',
			                '</th>',
		            	'</tr>'
					...
					strBox=[
						'<tr>',
				            ...
				            '<th>'+ret.actLoca+'</th>',
				            '<th>'+ret.SignPeople.length+'</th>',
			                '<th><button type="button" class="btn btn-success btn-xs">已结束</button></th>',
			                '<th>',			                	
			                	'<a href="#signCheck_Checkpeople?id='+ret.id+'"><button type="button" class="btn btn-primary btn-sm">查看签到结果</button></a>',
			                '</th>',
		            	'</tr>'
