...
KJT.component.saveMoney=(function($,window,undefined){
	...
	obj.prototype={
		...
		//获取url中的id
		getId:function(){
			var subAccount=window.location.href.split('?id=')[1];
		...
		strLink:function(ret,id){
			...
			if(this.subAccount==undefined){
				strBox=['<tr>'
		                ...
		                ,'<th>'+ret.rate+'</th>'
		                ,'<th>'
		                	,'<a><button type="button" class="btn KJT_btn btn-sm">选择</button></a><a class="info"></a>'
		                ,'</th>'
		            ,'</tr>'];
				...
				strBox=['<tr>'
	                ...
	                ,'<th>'+ret.rate+'</th>'
	                ,'<th>'
	                	,'<a href="#saveMoneyAmount?a='+ret.role+'?a='+ret.rate+'?a='+this.subAccount+'"><button type="button" class="btn KJT_btn btn-sm">选择</button></a>'
	                ,'</th>'
	            ,'</tr>'];
		    }
			return strBox.join('');
